...
const Register = () => {
    ...
    const handleChange = (e, required = false, label = "", pastedValue = "") => {
        ...
        const specialCharRegex =
            name === "dateOfBirth" || name === "presentPincode" || name === "mobileNumber"
                ? start_with_char_or_number
                : start_with_char;
    ...
    function returnControls(field) {
        ...
        if (field.type === "select") {
            ...
            return (
                <div className={colClass} key={field.id} style={{ position: "relative" }}>
                    <label htmlFor={field.id} className="form-label fw-medium required-label">
                        {field.label}
                    </label>
                    ...
                    <select
                        ...
                        name={field.name}
                        value={fieldValue}
                        onChange={(e) => formHandlers.handleChange(e, field.required, field.label)}
                        onBlur={(e) => handleOnBlur(e, field.required, field.label)}
                        className={`form-select ${hasError ? "hasError" : fieldValue ? "is-valid" : ""}`}
                    >
        ...
        if (["text", "tel", "email", "url", "number", "date", "password"].includes(field.type)) {
            return (
                <CustomInputField
                    key={field.id}
                    field={field}
